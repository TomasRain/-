...
> 运行方式请参考《[运行指南](RUNNING.md)》，其中包含命令行演示、Serper 实时搜索与 FastAPI 网页端的启动步骤。
...
### 当前实现（离线演示 + Serper 可选）
本仓库已提供一个最小可跑的命令行管线，并附带 FastAPI Web 入口：

- **检索**：默认使用内置演示数据（无需网络），可切换为 Serper.dev API。
- **正文获取**：演示数据直接包含内容；真实抓取时会尝试 `requests` + `trafilatura`。
- **摘要**：基于分句的轻量摘要，便于快速验证流程；后续可接入 LLM。
- **性能增强**：正文抓取与补全摘要均采用线程池并行，避免串行等待；同一关键词/源/条数查询会命中内存缓存（默认 10 分钟，LRU 32 条）。
- **输出**：终端展示汇总要点、指标与每条新闻的简短摘要，支持 JSON 输出用于调试或集成；Web 端可视化展示摘要、原文链接与实时/缓存命中状态。
...
访问 http://localhost:8000 ，输入关键词即可体验：

- **离线示例模式**：无需网，快速查看聚合与摘要效果。
- **Serper 模式**：设置 `SERPER_API_KEY` 后可实时拉取新闻，并通过 Web 端查看摘要与原文链接。
- **UI 提示**：顶部展示“实时拉取/命中缓存”与耗时指标，统计原始请求、去重数、正文抓取数与摘要数，便于观察性能与覆盖度。
